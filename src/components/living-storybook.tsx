...
export function LivingStorybook({
  ...
  const coreCompletedCount = coreBadges.filter((b) => {
    const p = badgeProgress.find((prog) => prog.badge_id === b.badge_id);
    return p?.status === "completed";
  }).length;
  // Strictly check that Stage 5 Gold Badge has been completed
  const isStage5Passed = badgeProgress.some((p) => Number(p.badge_id) === 5 && p.status === "completed");
  const isAllCoreStagesCompleted = isStage5Passed;
  const currentUser = getCurrentUser();
  const studentName = currentUser?.fullName || "Student";

  const displayedBadges = activePathwayTab === "core" ? coreBadges : teacherBadges;

  // Find currently active badge stage within the active tab
  const activeBadgeId =
    badgeProgress.find((p) => {
      const isTabMatch = activePathwayTab === "core" ? p.badge_id <= 5 : p.badge_id > 5;
      return isTabMatch && p.status === "in_progress";
    })?.badge_id ||
    displayedBadges[0]?.badge_id ||
    1;
  ...
  const handleTabSwitch = (tab: "core" | "teacher") => {
    ...
    const targetList = tab === "core" ? coreBadges : teacherBadges;
    const nextBadgeId = targetList[0]?.badge_id || 1;
  ...
  // Selected badge data
  const currentBadge =
    displayedBadges.find((b) => b.badge_id === selectedBadgeId) ||
    displayedBadges[0] ||
    badges[0] || {
      badge_id: 1,
  ...
  return (
    <div className="w-full space-y-6">
      {/* ── 1. Clean Minimal Storybook Header ───────────────── */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        {isAllCoreStagesCompleted && (
          <Button
            ...
            className="h-9 px-4 rounded-xl bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-600 hover:to-yellow-600 text-slate-950 font-black text-xs shadow-sm shadow-amber-500/20 flex items-center gap-2 cursor-pointer"
            title="View & Print Official Star Reader Certificate"
          >
      ...
      {/* ── 2. One-Click Pathway Tab Switcher (Core vs Teacher-Made Badges) ─── */}
      <div className="flex flex-wrap items-center p-1 bg-slate-100/90 rounded-2xl w-full sm:w-fit border border-slate-200/60 shadow-2xs gap-1">
        <button
          type="button"
        ...
        >
          <span>🌟 Core DepEd Stages ({coreBadges.length})</span>
        </button>
        ...
        >
          ...
          <span>Teacher Quests ({teacherBadges.length})</span>
          {teacherBadges.length > 0 && (
            <span className="text-[9px] bg-indigo-50 text-indigo-700 font-black px-1.5 py-0.2 rounded-full border border-indigo-200">
              Custom
            </span>
          )}
        </button>
      ...
      {/* ── 3. Empty State If No Teacher Badges Created Yet ─────────────────── */}
      {activePathwayTab === "teacher" && teacherBadges.length === 0 ? (
        <div className="bg-white rounded-3xl p-10 sm:p-14 border border-slate-200/80 shadow-sm text-center space-y-4 max-w-2xl mx-auto">
          <div className="w-16 h-16 rounded-3xl bg-indigo-50 border border-indigo-100 text-indigo-600 flex items-center justify-center text-3xl mx-auto shadow-xs">
            🏫
          </div>
          <div>
            <h3 className="text-base font-black text-slate-900">No Teacher Quests Available Yet</h3>
            <p className="text-xs text-slate-500 font-medium max-w-md mx-auto mt-1 leading-relaxed">
              Your teacher has not assigned custom curriculum badges for {currentUserSection} yet. Continue reading your Core DepEd Stages to unlock mastery seals!
            </p>
          </div>
          <Button
            onClick={() => handleTabSwitch("core")}
            className="bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl px-5 py-2.5 shadow-xs cursor-pointer"
          >
            <span>View Core DepEd Stages</span>
            <ArrowRight className="w-4 h-4 ml-1.5" />
          </Button>
        </div>
      ) : (
        /* ── 4. The Main Interactive Living Storybook Canvas ───────────────── */
        <div className="relative rounded-3xl bg-gradient-to-b from-blue-900 via-slate-900 to-indigo-950 p-3 sm:p-6 shadow-2xl border-4 border-amber-900/30 overflow-hidden">
          ...
          <div className="relative bg-[#fffdfa] rounded-2xl shadow-[0_12px_40px_rgba(0,0,0,0.35)] border border-amber-100/80 flex flex-col overflow-hidden">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-amber-200/70 min-h-[460px]">
              ...
              <div className="p-6 sm:p-8 flex flex-col justify-between relative bg-gradient-to-br from-amber-50/40 via-white to-blue-50/30">
                ...
                <div className="flex items-center justify-between text-xs font-bold text-slate-400 mb-2">
                  ...
                  <span>Page {(currentBadge.badge_order || 1) * 2 - 1}</span>
                </div>
                  ...
                  >
                    ...
                    <g transform="translate(180, 115)">
                      ...
                      <rect
                        ...
                        width="10"
                        height="10"
                        fill={storyVitality >= 80 ? "#FDE047" : "#94A3B8"}
                        stroke="#78350F"
                        strokeWidth="1"
                      />
                      <rect
                    ...
                    <g transform="translate(70, 140)">
                      <rect x="16" y="35" width="8" height="25" fill="#78350F" rx="2" />
                      <circle cx="20" cy="25" r="22" fill={storyVitality >= 20 ? "#16A34A" : "#64748B"} />
                      <circle cx="12" cy="18" r="14" fill={storyVitality >= 20 ? "#22C55E" : "#94A3B8"} />
                      <circle cx="28" cy="20" r="12" fill={storyVitality >= 20 ? "#15803D" : "#475569"} />
                    </g>
                    <g transform="translate(290, 150)">
                      <rect x="12" y="28" width="6" height="20" fill="#78350F" rx="2" />
                      <circle cx="15" cy="20" r="18" fill={storyVitality >= 20 ? "#16A34A" : "#64748B"} />
                      <circle cx="10" cy="14" r="12" fill={storyVitality >= 20 ? "#22C55E" : "#94A3B8"} />
                    </g>
                  </svg>
                </div>
                ...
                <div className="mt-4 space-y-1.5">
                  <div className="flex items-center justify-between text-xs font-bold">
                    <span className="text-slate-700 flex items-center gap-1">
                      ...
                      <span>Realm Vitality</span>
                    </span>
                    <span className="text-blue-700 font-black">{storyVitality}%</span>
                  </div>
                  <div className="w-full bg-slate-200/80 rounded-full h-2 overflow-hidden">
                    <div
                      className="bg-gradient-to-r from-amber-400 via-emerald-400 to-blue-500 h-2 rounded-full transition-all duration-700"
                      style={{ width: `${storyVitality}%` }}
                    />
                  </div>
                </div>
              </div>

              {/* ── RIGHT PAGE: Chapter Quest Path & Lessons ───────────────── */}
              <div
                className={`p-6 sm:p-8 flex flex-col justify-between relative bg-gradient-to-bl from-blue-50/20 via-white to-amber-50/20 transition-all duration-300 ${
                  isPageTurning ? "opacity-30 scale-[0.99] filter blur-[1px]" : "opacity-100 scale-100"
                }`}
              >
                <div>
                  {/* Page Header */}
                  <div className="flex items-center justify-between text-xs font-bold text-slate-400 mb-2">
                    <span className="text-[11px] font-black uppercase tracking-wider text-blue-700">
                      {activePathwayTab === "core"
                        ? `CHAPTER ${currentBadge.badge_order || 1} · ${currentBadge.badge_name}`
                        : `TEACHER QUEST · ${currentBadge.badge_name}`}
                    </span>
                    <span>Page {(currentBadge.badge_order || 1) * 2}</span>
                  </div>
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-blue-600 via-indigo-600 to-blue-700 text-white shadow-lg shadow-blue-500/20 relative overflow-hidden">
                        <div className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-blue-200 mb-1">
                        ...
                        <h3 className="text-base sm:text-lg font-black tracking-tight text-white mb-1.5">
                          {isStageBadgeMastered
                            ...
                            : currentActiveLesson
                            ? currentActiveLesson.lesson_title
                            : currentBadge.badge_name}
                        </h3>
                        <p className="text-xs text-blue-100 leading-relaxed mb-4 line-clamp-2">
                          {isStageBadgeMastered
                            ...
                            : allStoriesDone
                            ? "You have completed all stories! Take the Stage Final Evaluation to earn your Achievement Seal and unlock the next stage!"
                            : currentActiveLesson
                            ? currentActiveLesson.lesson_description ||
                              "Read through the story passage and take the comprehension quiz to advance."
                            : "Your teacher has created this custom quest for your class. Stories will be published here soon!"}
                        </p>
                        ...
                        <div className="flex flex-wrap items-center gap-2.5">
                          {allStoriesDone && !isStageBadgeMastered ? (
                            ...
                            >
                              <Button
                                size="default"
                                className="w-full sm:w-auto h-10 px-5 rounded-xl bg-amber-400 hover:bg-amber-300 text-amber-950 font-black text-xs shadow-md shadow-amber-500/30 flex items-center justify-center gap-2 transition-all duration-200 animate-bounce cursor-pointer"
                              >
                            ...
                            >
                              <Button
                                size="default"
                                className="w-full sm:w-auto h-10 px-5 rounded-xl bg-amber-400 hover:bg-amber-300 text-amber-950 font-black text-xs shadow-md shadow-amber-500/30 flex items-center justify-center gap-2 transition-all duration-200 cursor-pointer"
                              >
                            ...
                            <span className="text-xs font-bold text-blue-200">Stories coming soon</span>
                          )}
                        </div>
                      </div>
                    );
                  ...
                  {/* ── Chapter Stepping Stones (Stories) ────────────────── */}
                  <div className="mt-5 space-y-2">
                    <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block mb-1">
                      Chapter Stories ({chapterLessons.length})
                    </span>
                    ...
                    <div className="space-y-2">
                      {chapterLessons.map((lesson, idx) => {
                        ...
                        if (isDone) {
                          isUnlocked = true;
                        } else if (activePathwayTab === "teacher" || currentBadge.badge_id > 5) {
                          if (idx === 0) {
                            isUnlocked = true;
                          } else {
                          ...
                          if (currentBadge.badge_order === 1) {
                            ...
                            const prevBadgeProg = badgeProgress.find(
                              (p) => p.badge_id === (currentBadge.badge_order || 1) - 1
                            );
                            isUnlocked = Boolean(
                          ...
                          >
                            ...
                            {isDone ? (
                              <div className="flex items-center gap-1.5">
                                <Link href={`/dashboard/lessons?lessonId=${lesson.lesson_id}`}>
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    className="h-7 px-2.5 text-[10px] font-bold rounded-lg border-emerald-300 text-emerald-800 bg-emerald-50 hover:bg-emerald-100 cursor-pointer"
                                  >
                                    Re-read
                                  </Button>
                                </Link>
                                <Link href={`/dashboard/quiz?lessonId=${lesson.lesson_id}`}>
                                  <Button
                                    size="sm"
                                    variant="outline"
                                    className="h-7 px-2.5 text-[10px] font-bold rounded-lg border-blue-200 text-blue-700 hover:bg-blue-50 cursor-pointer"
                                  >
                                    Retake
                                  </Button>
                                </Link>
                              ...
                              >
                                <Button
                                  size="sm"
                                  className="h-7 px-3.5 text-[11px] font-black rounded-lg bg-blue-600 hover:bg-blue-700 text-white shadow-xs cursor-pointer"
                                >
                                  Read Story
                                </Button>
                              </Link>
                        ...
                        <div className="flex items-center justify-between gap-3">
                            ...
                            <div>
                              ...
                              <span className="text-[10px] text-slate-500 font-medium">
                                {isStageBadgeMastered
                                  ...
                                  : allStoriesDone
                                  ? `🌟 Ready! Score ≥${currentBadge.required_passing_score}% to earn Seal & unlock Next Chapter.`
                                  : chapterLessons.length === 0
                                  ? "🔒 Locked (No stories published for this quest yet)"
                                  : "🔒 Locked (Complete all stories in this chapter first)"}
                              </span>
                            </div>
                          </div>
                          ...
                          {allStoriesDone ? (
                            ...
                            >
                              <Button
                                size="sm"
                                className={`h-8 px-3.5 text-xs font-black rounded-xl shadow-sm cursor-pointer ${
                                  isStageBadgeMastered
            ...
            {/* ── 5. Magical Seals Tray (Dynamically reflects Core or Teacher Quests) ── */}
            <div className="bg-gradient-to-r from-amber-100/90 via-amber-50 to-amber-100/90 border-t-2 border-amber-200/80 p-4 sm:p-5">
              <div className="text-center mb-3">
                <span className="text-[11px] font-black uppercase tracking-widest text-amber-900/80">
                  {activePathwayTab === "core"
                    ? "✨ Collect All 5 Core DepEd Achievement Seals ✨"
                    : `✨ Teacher Classroom Achievement Seals (${teacherBadges.length}) ✨`}
                </span>
              </div>

              <div
                className={`grid gap-2 sm:gap-4 max-w-2xl mx-auto items-center ${
                  displayedBadges.length <= 5
                    ? `grid-cols-${Math.max(1, displayedBadges.length)}`
                    : "grid-cols-5"
                }`}
                style={{
                  gridTemplateColumns: `repeat(${Math.min(5, Math.max(1, displayedBadges.length))}, minmax(0, 1fr))`,
                }}
              >
                {displayedBadges.map((badge) => {
                  const prog = badgeProgress.find((p) => p.badge_id === badge.badge_id);
                  const isCompleted = prog?.status === "completed";
                  const isTeacherBadge = badge.badge_id > 5 || activePathwayTab === "teacher";
                  const isInProg =
                    isTeacherBadge ||
                    prog?.status === "in_progress" ||
                    (!prog && badge.badge_id === displayedBadges[0]?.badge_id);
